...
description: Thermo-nuclear security audit (injection, auth bypass, secrets exposure, boundary validation, OWASP). Invoked via Task after a parent gathers diff and file contents. Loads the rubric from the `thermo-nuclear-security-review` skill.
mode: subagent
temperature: 0.1
tools:
  read: true
