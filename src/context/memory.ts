...
export class MemoryManager {
  ...
  async addMemory(
    ...
    const entry: MemoryEntry = {
      ...
      provenance: {
        ...
        timestamp: now,
        confidence: options?.confidence ?? (options?.source === "inferred" ? 0.7 : 1.0),
  ...
  searchMemories(
    ...
    const types = options?.type ? [options.type] : (Object.keys(this.store) as MemoryType[])
  ...
  getStats(): {
    ...
    const avgConfidence =
      all.length > 0
        ? all.reduce((sum, m) => sum + m.provenance.confidence, 0) / all.length
        : 0
  ...
  private applyConfidenceDecay(entry: MemoryEntry): MemoryEntry {
    ...
    if (entry.provenance.source === "inferred") {
      ...
      const decayFactor = Math.pow(1 - this.config.confidenceDecayRate, daysSinceCreation)
      entry.provenance.confidence *= decayFactor
    } else if (entry.provenance.source === "agent") {
      // Agent memories decay slower
      const decayFactor = Math.pow(1 - this.config.confidenceDecayRate * 0.5, daysSinceCreation)
      entry.provenance.confidence *= decayFactor
    }
   ...
   */
  async archiveOldMemories(daysThreshold: number = 30): Promise<number> {
    const now = new Date()
    let archived = 0
   ...
   */
  getSummary(maxItems: number = 5): string {
...
export function getMemoryManager(config?: Partial<ContextConfig>): MemoryManager {
  if (!defaultManager) {
