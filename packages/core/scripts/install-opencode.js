...
async function install(targetDir, silent = false) {
    ...
    const distOpenCodeDir = path.join(packageRoot, "opencode");
    const targetOpenCodeDir = targetDir;
    if (!fs.existsSync(distOpenCodeDir)) {
        if (!silent) {
            console.error(
                "❌ Error: opencode/ directory not found in @ai-eng-system/core package.",
            );
        }
        process.exit(1);
    }
    ...
    cleanAiEngSkills(targetOpenCodeDir, distOpenCodeDir, silent);
    const distSkillDir = path.join(distOpenCodeDir, "skill");
    ...
    // Claude hooks installation moved to separate script - not part of OpenCode installer
    if (!silent) {
        console.log(`
