/**
 * OpenCode adapter — hybrid OpenAI + Kimi routing.
 *
 ...
 * Kimi is OpenAI-compatible at api.moonshot.cn/v1.
 *
 * Default mapping:
...
const KIMI = process.env.OPENCODE_MODEL ?? "kimi-k2.6";
const OPENAI_HEAVY = process.env.OPENCODE_MODEL_PLANNER ?? "gpt-5.5";

export const OPENCODE_MODEL_FAMILY = {
    [SubagentRole.LOOKUP]: process.env.OPENCODE_MODEL_LOOKUP ?? KIMI,
    [SubagentRole.WORK]: process.env.OPENCODE_MODEL_WORK ?? KIMI,
    [SubagentRole.PLANNER]: OPENAI_HEAVY,
    [SubagentRole.DEBUGGER]: process.env.OPENCODE_MODEL_DEBUGGER ?? KIMI,
    [SubagentRole.REFACTORER]:
        process.env.OPENCODE_MODEL_REFACTORER ?? OPENAI_HEAVY,
} satisfies Record<SubagentRole, string>;

export const opencodeAdapter: RouterAdapter = {
    name: "opencode",
    models: OPENCODE_MODEL_FAMILY,
    execute: createStubExecute("opencode"),
};
