...
class EvaluationRunner {
    ...
    async runEvaluation(
        resultsDir: string,
    ): Promise<{ status: string; evaluations: number }> {
        if (!this.client) {
        ...
        if (this.config.dryRun) {
            console.log(
                "🔍 DRY RUN MODE - No evaluation API calls will be made",
            );
        ...
        for (const [taskId, responses] of Object.entries(taskResponses)) {
            if (
                responses.baseline.length > 0 &&
                responses.enhanced.length > 0
            ) {
                for (
                    let i = 0;
                    i <
                    Math.min(
                        responses.baseline.length,
                        responses.enhanced.length,
                    );
                    i++
                ) {
    ...
    private generateMockEvaluation(taskId: string): Record<string, unknown> {
        ...
        const enhancedScore = (baseline: number) => {
            if (Math.random() < 0.65) {
                return Math.min(
                    5,
                    baseline + Math.floor(Math.random() * 2) + 1,
                );
            }
