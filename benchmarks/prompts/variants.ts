...
export class PromptVariantGenerator {
    ...
    constructor(config: CollectionConfig = {}) {
        this.config = { num_variants: 3, ...config };
    }
     ...
     */
    private createRephrasedVariant(
        template: string,
        _task: BenchmarkTask,
    ): PromptVariant {
        const rephrasings = [
        ...
        const rephrasedTask =
            rephrasings[Math.floor(Math.random() * rephrasings.length)];

        const modifications = [
            `Rephrased task: "${originalTask}" → "${rephrasedTask}"`,
        ];
    ...
    private createContextVariant(
        ...
        if (hasContext) {
            ...
            const minimalTemplate = template.replace(
                /\{\{#if context\}\}[^}]*\{\{\/if\}\}/g,
                "",
            );
...
export function generatePromptVariants(
    ...
    task_id: string;
    variants: { baseline: PromptVariant[]; enhanced: PromptVariant[] };
}[] {
    const results: {
        task_id: string;
        variants: { baseline: PromptVariant[]; enhanced: PromptVariant[] };
    ...
    for (const task of tasks) {
        const baselineVariants = new PromptVariantGenerator(
            config,
        ).generateVariants(task, templates.baseline);
        const enhancedVariants = new PromptVariantGenerator(
            config,
        ).generateVariants(task, templates.enhanced);
