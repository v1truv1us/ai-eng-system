#!/usr/bin/env bun
/**
 * Unified Validation Runner for Ferg Engineering System
...
class ValidationRunner {
  ...
  constructor(options: {
    ...
    this.configFile = options.configFile || path.join(__dirname, "config.json");
  ...
  async run(): Promise<void> {
    console.log("🚀 Starting Ferg Engineering System Validation");
    ...
    if (this.categoryFilter) {
      filteredTasks = Object.fromEntries(
        Object.entries(tasks).filter(([, task]) => task.category === this.categoryFilter)
      );
      console.log(`📋 Filtered to ${Object.keys(filteredTasks).length} tasks in category: ${this.categoryFilter}`);
    }

    // Load prompts
    console.log("📋 Loading prompt templates...");
    const prompts = await this.loadPrompts();
    console.log(`✓ Loaded ${Object.keys(prompts.baseline).length} baseline and ${Object.keys(prompts.enhanced).length} enhanced prompt templates`);
    ...
    console.log(`📈 Report saved to: ${this.config.output.results_dir}/statistical_report.md`);
    ...
    if (analysisResult.summary) {
      ...
      console.log(`   Significant Improvements: ${summary.significant_improvements || 0}`);
      console.log(`   Effect Size: ${summary.effect_size?.toFixed(2) || 'N/A'}`);
    }
  ...
  private async loadPrompts(): Promise<PromptTemplates> {
    ...
    try {
      ...
      for (const file of baselineFiles) {
        if (file.endsWith(".md")) {
          const content = await fs.readFile(path.join(baselineDir, file), "utf-8");
    ...
    try {
      ...
      for (const file of enhancedFiles) {
        if (file.endsWith(".md")) {
          const content = await fs.readFile(path.join(enhancedDir, file), "utf-8");
          const promptType = path.basename(file, ".md");
  ...
  private async collectResponses(): Promise<void> {
    console.log("📝 Starting response collection with realistic mock data...");
    ...
    const responses = await this.generateMockResponses(requests, this.config.output.results_dir);

    console.log(`✓ Generated ${responses.length} realistic mock responses`);
  }

  private createCollectionRequests(tasks: Record<string, TaskData>, prompts: PromptTemplates): Array<{
    task_id: string;
    ...
    for (const [category, categoryTasks] of Object.entries(tasksByCategory)) {
      ...
      for (const [taskId, taskData] of categoryTasks) {
        ...
        for (const promptType of ["baseline", "enhanced"] as const) {
          ...
          if (!promptTemplate) {
            console.warn(`⚠️  No ${promptType} prompt template found for category: ${category}`);
            continue;
          }

          // Create variants
          for (let variantNum = 0; variantNum < targetResponsesPerType; variantNum++) {
            const variantId = `v${variantNum}`;

            // Populate template with task data
            const prompt = this.populateTemplate(promptTemplate, taskData);
  ...
  private populateTemplate(template: string, taskData: TaskData): string {
    ...
    result = result.replace("{{language}}", taskData.language || "javascript");
    ...
    for (const request of requests) {
      try {
        ...
        if (request.task_data.task) {
          ...
          if (category === "creative" && request.task_data.task.toLowerCase().includes("component")) {
            responseText = responseText.replace("{{component}}", "reusable React component");
          }
        }
        ...
        const responseData = {
          ...
          metadata: {
            ...
            category: category,
            prompt_type: request.prompt_type,
            quality: request.prompt_type === "enhanced" ? "high" : "standard"
          },
        };
        ...
        console.log(`  ✓ Generated realistic response: ${request.task_id} ${request.prompt_type} ${request.variant_id} (${responseText.length} chars)`);

      } catch (error) {
        console.error(`❌ Error generating response for ${request.task_id}: ${error}`);
      }
    }
  ...
  private async runEvaluations(): Promise<void> {
    ...
    const result = await runner.runEvaluation(path.join(__dirname, "results"));

    console.log(`  Evaluations completed: ${result.evaluations} pairs processed`);
  }
  ...
  private async generateAnalysis(): Promise<any> {
    ...
    const evaluationFiles = evalFiles.filter(f => f.endsWith('_eval.json'));
    ...
    for (const file of evaluationFiles) {
      try {
        ...
        console.warn(`⚠️  Error reading evaluation file ${file}: ${error}`);
      }
    }
    ...
    const improvements = evaluations.map(e => e.enhanced_score - e.baseline_score);
    ...
    const pooledStd = Math.sqrt(
      ((baselineScores.length - 1) * this.stdDev(baselineScores) ** 2 +
       (enhancedScores.length - 1) * this.stdDev(enhancedScores) ** 2) /
    ...
    const significantImprovements = evaluations.filter(e => e.enhanced_score > e.baseline_score + 0.5).length;

    console.log(`📊 Analysis complete: ${evaluations.length} evaluations, ${significantImprovements} significant improvements`);

    // Generate markdown report
    const report = this.generateMarkdownReport(evaluations, avgBaseline, avgEnhanced, avgImprovement, cohenD, significantImprovements);

    // Save report
    const reportPath = path.join(this.config.output.results_dir, 'statistical_report.md');
  ...
  private generateMarkdownReport(
    evaluations: Array<{task_id: string; baseline_score: number; enhanced_score: number; winner: string}>,
    avgBaseline: number,
    avgEnhanced: number,
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case "--category":
        ...
        break;
      case "--num-variants":
        options.numVariants = parseInt(args[++i]);
        break;
      case "--help":
      case "-h":
        printHelp();
...
function printHelp() {
  console.log(`
Ferg Engineering System Validation Runner
