...
function findOpenCodeConfig(startDir) {
  ...
  const projectConfigPath = path.join(startDir, ".opencode", "opencode.jsonc");
...
async function installClaudeHooks(targetDir, silent = false) {
  const canonicalHooksDir = path.join(packageRoot, "plugins", "ai-eng-system", "hooks");
  const targetHooksDir = path.join(targetDir, ".claude", "hooks");
  if (!fs.existsSync(canonicalHooksDir)) {
    if (!silent) {
      console.log("  ℹ️  No hooks found in plugins/ai-eng-system/hooks/ (skip)");
    }
    return;
  }
  ...
  if (!silent) {
    ...
    console.log("    \uD83D\uDCDD Hooks enable automatic prompt optimization (+45-115% quality)");
...
async function install(targetDir, silent = false) {
  ...
  cleanNamespacedDirectory(targetOpenCodeDir, "command", NAMESPACE_PREFIX, silent);
  ...
  if (fs.existsSync(commandsSrc)) {
    ...
    if (!silent)
      console.log(`  ✓ command/${NAMESPACE_PREFIX}/ (${commandCount} commands)`);
  }
  cleanNamespacedDirectory(targetOpenCodeDir, "agent", NAMESPACE_PREFIX, silent);
  ...
  if (fs.existsSync(agentsSrc)) {
    ...
    if (!silent)
      console.log(`  ✓ agent/${NAMESPACE_PREFIX}/ (${agentCount} agents)`);
  }
  ...
  if (fs.existsSync(distSkillDir)) {
    ...
    if (!silent)
      console.log(`  ✓ skill/ (${skillCount} skills)`);
  }
...
async function main() {
  ...
  if (isPostInstall) {
    ...
    const configResult = findOpenCodeConfig(cwd);
    if (!configResult) {
      console.error("❌ Error: opencode.jsonc not found in .opencode/ or ~/.config/opencode/");
      console.error("   Create .opencode/opencode.jsonc in your project, or use ~/.config/opencode/opencode.jsonc for global installation");
      process.exit(1);
    }
    if (!isPluginReferenced(configResult.path)) {
      console.error("❌ Error: ai-eng-system is not referenced in opencode.jsonc plugin list");
      console.error("   Add 'ai-eng-system' to the plugin array in opencode.jsonc");
      process.exit(1);
    }
    const targetDir = path.dirname(configResult.path);
