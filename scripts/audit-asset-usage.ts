...
function getLastModified(filePath: string): { date: string; author: string } {
  const log = runGit(["log", "-1", "--format=%H%x00%ai%x00%an", "--", filePath]);
...
> {
  const assets: Array<{ name: string; type: "command" | "agent" | "skill"; path: string }> = [];
  ...
  if (existsSync(commandsDir)) {
    ...
    for (const f of files.filter((x) => x.endsWith(".md"))) {
      assets.push({ name: basename(f, ".md"), type: "command", path: join(commandsDir, f) });
    }
  }
  ...
  if (existsSync(agentsDir)) {
    ...
    for (const f of files.filter((x) => x.endsWith(".md"))) {
      assets.push({ name: basename(f, ".md"), type: "agent", path: join(agentsDir, f) });
    }
  }
...
function findReferences(targetName: string, allAssets: Array<{ path: string; name: string; type: string }>): string[] {
  const refs: string[] = [];
  ...
  for (const asset of allAssets) {
    try {
      ...
      if (patterns.some((p) => p.test(content)) && asset.name !== targetName) {
  ...
  for (const dir of [".claude", ".pi"]) {
    try {
      const entries = readdirSync(join(process.cwd(), dir), { recursive: true, withFileTypes: true });
...
async function main(): Promise<void> {
  ...
  console.log(`Auditing assets (since=${since}, stale-days=${args.staleDays})...\n`);
  ...
  // Detail table
  console.log(`=== DETAIL ===`);
  console.log(`${"Type".padEnd(10)} ${"Name".padEnd(30)} ${"Last Mod".padEnd(12)} ${"Commits".padEnd(8)} ${"Refs".padEnd(5)} ${"Status"}`);
  console.log("-".repeat(90));

  for (const r of reports) {
    const status = [r.orphan ? "ORPHAN" : "", r.stale ? "STALE" : ""].filter(Boolean).join("+") || "OK";
  ...
  if (candidates.length > 0) {
    ...
    for (const r of candidates) {
      console.log(`  ${r.type}/${r.name} — last modified ${r.last_modified}, ${r.commit_count_12mo} commits in 12mo`);
    }
  }
}
