...
async function gradeAssertion(
    ...
    // Detect negation
    const hasNegation =
        /\b(does not|doesn't|must not|should not|no|never|without)\b/i.test(
            assertion,
        );
    ...
    const sectionMatch = lowerAssertion.match(
        /section titled ['"]([^'"]+)['"]/,
    );
    if (sectionMatch) {
        const sectionName = stripMarkdown(sectionMatch[1]);
        const found =
            cleanOutput.includes(sectionName) ||
            rawOutput.includes("## " + sectionName);
        if (hasNegation) {
            return {
                text: assertion,
                passed: !found,
    ...
    if (boldMatch) {
        ...
        if (hasNegation) {
            return {
                text: assertion,
                passed: !found,
                evidence: found ? `Found ${boldText}` : `${boldText} not found`,
            };
        }
        return {
            text: assertion,
    ...
    const countMatch = lowerAssertion.match(
        /at least (\d+) .*?(?:checkbox|- \[ \]|item)/i,
    );
    if (countMatch) {
        ...
        return {
            text: assertion,
            passed: found,
            evidence: `Found ${count} checkboxes (need ${minCount})`,
        };
    }
    ...
    if (hasNegation) {
        ...
        const forbiddenFound = quotedStrings.filter((q) =>
            cleanOutput.includes(q),
        );
        if (quotedStrings.length > 0) {
            ...
            return {
                ...
                passed,
                evidence: passed
                    ? `None of the forbidden phrases found`
                    : `Found forbidden phrases: ${forbiddenFound.slice(0, 3).join(", ")}`,
            };
        }
        ...
        const words = lowerAssertion
            .replace(
                /\b(does not|doesn't|must not|should not|no|never|without|contain|include|have|use|assume|the|output|it|is|are|be|being|been|a|an|and|or|but|with|for|from|than|this|that|these|those|word|words)\b/g,
                " ",
            )
            .split(/\s+/)
            .filter((w) => w.length > 3);
        ...
        return {
            ...
            passed,
            evidence: passed
                ? `Forbidden concepts not found`
                : `Found: ${found.slice(0, 5).join(", ")}`,
        };
    }
    ...
    if (quotedStrings.length > 0) {
        ...
        const passed =
            found.length >= Math.max(1, Math.floor(quotedStrings.length * 0.5));
        return {
            text: assertion,
            passed,
            evidence: passed
                ? `Matched ${found.length}/${quotedStrings.length} quoted phrases`
                : `Only matched ${found.length}/${quotedStrings.length}`,
        };
    }

    // Fallback: keyword matching with unit-aware expansion
    let keywordText = lowerAssertion.replace(/["'[\](){}]/g, "");
    ...
    return {
        ...
        passed,
        evidence: passed
            ? `Found ${matches}/${keywords.length} keywords`
            : `Only found ${matches}/${keywords.length} keywords`,
    };
}
...
async function gradeSkill(
    ...
    for (const evalCase of evalsJson.evals) {
        ...
        if (existsSync(outputFile)) {
            ...
            console.warn(
                `  No output file for eval ${evalCase.name}: ${outputFile}`,
            );
        }
        ...
        results.push({
            ...
            summary: {
                ...
                failed,
                total: passed + failed,
                pass_rate: passed / (passed + failed) || 0,
            },
        });
    }
...
async function gradeAll(workspace: string): Promise<void> {
    ...
    const skillDirs = entries.filter(
        (e) => e.isDirectory() && e.name !== "AGENTS.md",
    );
    ...
    const benchmark = {
        run_summary: {
            ...
            total_assertions: allResults.reduce(
                (s, r) => s + r.aggregate.total,
                0,
            ),
            overall_pass_rate:
                allResults.reduce((s, r) => s + r.aggregate.passed, 0) /
                Math.max(
                    1,
                    allResults.reduce((s, r) => s + r.aggregate.total, 0),
                ),
    ...
    console.log(
        `Overall pass rate: ${(benchmark.run_summary.overall_pass_rate * 100).toFixed(1)}%`,
    );
    console.log(`\nPer-skill breakdown:`);
    for (const s of benchmark.run_summary.skills) {
        const status =
            s.pass_rate >= 0.8 ? "✅" : s.pass_rate >= 0.5 ? "⚠️" : "❌";
        console.log(
            `  ${status} ${s.name}: ${s.passed}/${s.passed + s.failed} (${(s.pass_rate * 100).toFixed(0)}%)`,
        );
    }
}
