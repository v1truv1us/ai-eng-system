...
function generateEvals(
    ...
    const hasWhenToUse = sections.some(
        (s) => s.includes("when to use") || s.includes("when"),
    );
    const hasProcess = sections.some(
        (s) => s.includes("process") || s.includes("workflow"),
    );
    const hasRules = sections.some(
        (s) => s.includes("rule") || s.includes("constraint"),
    );
    ...
    evals.push({
        ...
        assertions: [
            ...
            hasRules
                ? "The output respects the skill's stated rules or constraints"
    ...
    evals.push({
        ...
        expected_output:
            "The skill handles vague input by either asking clarifying questions or providing bounded guidance.",
        assertions: [
    ...
    if (hasExample || sections.length > 5) {
        evals.push({
            ...
            expected_output:
                "The skill scales its guidance to a multi-team, multi-service context without losing specificity.",
            assertions: [
                "The output addresses complexity without being vague",
                "The output provides concrete next steps",
                sections.some((s) => s.includes("scale") || s.includes("team"))
                    ? "The output includes team or scaling guidance"
...
async function main(): Promise<void> {
    ...
    console.log(
        `Created ${created} eval files, skipped ${skipped} (already exist)`,
    );
}
