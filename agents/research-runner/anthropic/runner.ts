...
async function main(): Promise<void> {
    ...
    console.error(
        `Running ${templates.length} template(s) via Anthropic driver…`,
    );

    const driver = await createDriver("anthropic");
    try {
        const results = await mapLimit(templates, 3, (t) =>
            runTemplate(driver, t, data.systemPrompt, query, agent),
        );

        const synthesis = await synthesize(query, results);
        const briefPath = writeBrief(query, results, "anthropic", synthesis);
