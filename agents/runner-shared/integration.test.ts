...
describe("runner integration patterns", () => {
  ...
  test("parseArgs + formatPrompt integration", () => {
    const argv = ["--templates", "A1,M2", "--agent", "reviewer", "my query"];
    const { positionals, flags } = parseArgs(argv, ["--templates", "--agent"]);
    ...
    const prompt = formatPrompt("You are a researcher.", "Find X.", query, agent);
