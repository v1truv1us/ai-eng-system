...
description: Thermo-nuclear performance audit (algorithmic complexity, memory, N+1 queries, network patterns, bundle size, scalability). Invoked via Task after a parent gathers diff and file contents. Loads the rubric from the `thermo-nuclear-performance-review` skill.
mode: subagent
temperature: 0.1
tools:
  read: true
